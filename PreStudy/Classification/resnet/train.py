...
def main():
    data_root = os.path.abspath(os.path.join(os.getcwd(), "D:/Software"))  # get data root path
    # image_path = os.path.join(data_root, "data_set", "Food501")  # data set path
    image_path = os.path.join(data_root, "data_set")  # data set path

    train_dir = os.path .join(image_path, "Food10_train")
    validation_dir = os.path.join(image_path, "Food10_val")
    ...
    batch_size = 16
    epochs = 2
    # number_classed need to change before train
    num_classes = 10
   ...
   # change num_class!!
    ...
    feature.trainable = False  # feature 所有权重都会被冻结，训练过程中也无法训练这些参数，training设置为True也不行
    feature.summary()
    ...
    for epoch in range(epochs):
        ...
        for step in train_bar:
            ...
            # print train process
            train_bar\
                .desc = "train epoch[{}/{}] loss:{:.3f}, acc:{:.3f}".format(epoch + 1,
                                                                                 epochs,
                                                                                 train_loss.result(),
