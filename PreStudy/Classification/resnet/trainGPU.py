...
def main():
    ...
    # image_path = os.path.join(data_root, "data_set", "food_data2")  # flower data set path
    image_path = os.path.join(data_root, "data_set")  # flower data set path
    train_dir = os.path.join(image_path, "Food20_train")
    validation_dir = os.path.join(image_path, "Food20_val")
    ...
    def process_train_img(img_path, label):
        try:
            ...
            print(f"Error processing file: {img_path} {label}")
    ...
    # 实例化模型
    feature = resnet50(num_classes=20, include_top=False)
